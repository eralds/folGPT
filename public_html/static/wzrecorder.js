function WzRecorder(config) {
    ...
    var responseAudio = new Audio();
    var usrTxt = document.getElementById("user");
    var srvTxt = document.getElementById("server");
    ...
    this.speech = function() {
		...
		request.onreadystatechange = function (e) {
			...
			if (request.readyState == 4)
			{
				...
				var wavData = request.response

                var blobURL = URL.createObjectURL(wavData);
                responseAudio.src = blobURL;
                responseAudio.play();
			}
		};
    ...
    this.talkToGpt = function() {
        var formData = new FormData();
        const jsonData = {
            lang: languageSelect.value,
            input: usrTxt.innerText
        };   
        formData.append('json', JSON.stringify(jsonData));
        var request = new XMLHttpRequest();
		request.onreadystatechange = function (e) {
			...
			if (request.readyState == 4)
			{
                ...
                srvTxt.innerHTML = data.server;
                self.speech();

			}
		};
  
        request.open("POST", "/gpt" ,true);
        request.send(formData);
    };
...
// var blob1 = e.target.response
// const source = audioCtx.createBufferSource();
// console.log(typeof blob1)
// console.log(typeof source.buffer)
// const encoder = new TextEncoder();
// const u = encoder.encode(blob1);
